...
import { getWhatsAppShareLink, getEmailShareLink, getLinkedInShareLink, getXShareLink, getLinkedInAddCertificationLink, buildLinkedInCertificationOptions } from '@/lib/shareCertificateService';
...
export default function ShareFallbackModal({
  ...
  // Nuevo: Agregar certificación al perfil de LinkedIn
  const handleAddToLinkedInProfile = () => {
    if (!token || !generatedAt) {
      // Fallback a compartir si no hay token/fecha
      handleLinkedIn();
      return;
    }

    const options = buildLinkedInCertificationOptions(
      { certificateUrl, studentName, courseName, validationUrl },
      generatedAt,
      token
    );
  ...
  const handleX = () => {
    window.open(getXShareLink({ certificateUrl, studentName, courseName, validationUrl }), '_blank', 'width=600,height=400');
    setShared(true);
  };
  ...
  // Vista compacta después de compartir
  if (shared) {
    return (
      <>
        {/* Overlay con blur */}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full animate-scale-in overflow-hidden">
          ...
          <div className="px-5 py-4">
            {/* Opción destacada: Agregar a LinkedIn */}
            {token && generatedAt && (
              <div className="mb-4">
                <button
                  onClick={handleAddToLinkedInProfile}
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-[#0A66C2] text-white hover:bg-[#004182] transition-colors"
                >
                  ...
                  <div className="flex-1 text-left">
                    <span className="font-semibold block">Agregar a mi Perfil</span>
                    <span className="text-xs opacity-80">Aparece en tu sección de Certificaciones</span>
                  </div>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-xs text-gray-500">o compartí en redes</span>
              <div className="flex-1 h-px bg-gray-200"></div>
            </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-gray-700">Copiar link de validación</span>
              </div>
              {copied ? (
